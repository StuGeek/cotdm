...
def plot_fig_nor_mean_lat(model_name_list, mean_lat_list_all_schemes):
    ...
    mean_lat_list_totoff = mean_lat_list_all_schemes[scheme_idx_totoff]
    nor_mean_lat_list = []
    for deploy_scheme in deploy_scheme_list:
        ...
        nor_mean_lat_list.append(
            [
                mean_lat_list[i] / mean_lat_list_totoff[i]
    ...
    for i in range(len(nor_mean_lat_list)):
        plt.bar(
            x - (1 - i) * width,
            nor_mean_lat_list[i],
            width=width,
    ...
    plt.yticks(np.arange(0, 0.7, 0.15), fontsize=10)
    plt.ylabel("Normalized latency", fontsize=10)
    plt.legend(bbox_to_anchor=(-0.1, 0.95, 0.91, 0.3), ncol=3, fontsize=9)
    plt.savefig("exp1_nor_mean_lat.png", bbox_inches="tight")
    print("Figure {} save path: {}".format("exp1_nor_mean_lat.png", os.path.dirname(os.path.abspath(__file__)) + "/exp1_nor_mean_lat.png"))


def plot_fig_nor_thr(model_name_list, thr_list_all_schemes):
    deploy_scheme_dict = global_config.DEPLOY_SCHEME_DICT
    deploy_scheme_list = ["Strawman", "DeepPlan", "CoTDM"]
    scheme_idx_totoff = deploy_scheme_dict["Totally-Offload"]
    thr_list_totoff = thr_list_all_schemes[scheme_idx_totoff]
    nor_thr_list = []
    for deploy_scheme in deploy_scheme_list:
        scheme_idx = deploy_scheme_dict[deploy_scheme]
    ...
    plt.grid(visible=True, axis="y", zorder=0)
    for i in range(len(nor_thr_list)):
        plt.bar(
            x - (1 - i) * width,
            nor_thr_list[i],
            width=width,
    ...
    plt.xticks(x, labels=model_name_list, fontsize=10)
    plt.yticks(np.arange(0, 4.5, 1), fontsize=10)
    plt.ylabel("Normalized throughput", fontsize=10)
    plt.legend(bbox_to_anchor=(-0.1, 0.95, 0.91, 0.3), ncol=3, fontsize=9)
    plt.savefig("exp1_nor_thr.png", bbox_inches="tight")
    print("Figure {} save path: {}".format("exp1_nor_thr.png", os.path.dirname(os.path.abspath(__file__)) + "/exp1_nor_thr.png"))
...
def plot_fig_exp1(model_name_list, mean_lat_list_all_schemes, thr_list_all_schemes, cost_list_all_schemes):
    ...
    plt.figure(1)
    plot_fig_nor_mean_lat(model_name_list, mean_lat_list_all_schemes)
    plt.figure(2)
    plot_fig_nor_thr(model_name_list, thr_list_all_schemes)
    plt.figure(3)
